...
class Model():
	...
	def __init__(self,DATASET):
		...
		self.var = (self.cov_mat[0][0] + self.cov_mat[1][1])/2
		self.cov_mat[0][0] =self.var
		self.cov_mat[1][1]=self.var
		self.cov_mat[0][1]=0
		self.cov_mat[1][0]=0
	def Plot_Classifier(self,RANGE,val):
		...
		while i<=RANGE[0][1]:
			...
			while j<=RANGE[1][1]:
				index=-1
				x = np.array([i,j])
				index = sf.calc_Gx_case1(x,self.mean_mat,self.var, self.prior)
	...
	def plot_pair(self,RANGE,val):
		...
		while i<=RANGE[0][1]:
			...
			while j<=RANGE[1][1]:
				index=-1
				x = np.array([i,j])
				index = sf.Gx_case1_pair(x,mean1, mean2, self.var, p1,p2)
				temp[index].append([i,j])
				j=j+val
			i=i+val
